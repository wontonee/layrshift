...
wp_register_ability('layrshift/prismshift-update-post-seo', [
    'label' => __('Update PrismShift Post SEO', 'layrshift'),
    'description' => __('Update PrismShift SEO fields on a post. Defaults to draft-only unless allow_publish is true.', 'layrshift'),
    ...
    'execute_callback' => __NAMESPACE__ . '\\prismshift_update_post_seo',
...
function prismshift_update_post_seo(array $input): array|WP_Error
{
    ...
    if ($post->post_status === 'publish' && !$allow_publish) {
        return new WP_Error(
            ...
            __('Updating PrismShift SEO on published posts requires allow_publish=true and explicit user approval.', 'layrshift')
        );
    }

    $fields = array_intersect_key(
        $input,
        array_flip(array('seo_title', 'meta_description', 'focus_keyword', 'noindex', 'canonical', 'schema_type'))
    );
